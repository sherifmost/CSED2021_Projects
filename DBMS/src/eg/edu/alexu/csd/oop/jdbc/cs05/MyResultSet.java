...
public class MyResultSet implements java.sql.ResultSet {
	...
	private Object[][] removenull(Object arr[][]) {
		...
		if(hasnull) {
			Object[][] newarr = new Object [arr.length][arr[0].length/2];
			int in = 0,jn=0;
			for(int i=0;i<arr.length;i++) {
				for(int j=1;j<arr[i].length;j+=2) {
					newarr[in][jn] = arr[i][j];
					jn++;
				}
				in++;
			}
	...
	public MyResultSet(Object[][] data,Database mydb,Statement stat) {
		this.data = removenull(data);//data;
		this.mydb = mydb;
		this.stat = stat;
	}
	...
	public int getInt(int columnIndex) throws SQLException {
		if(getObject(columnIndex) instanceof Integer) {
	...
	public boolean absolute(int row) throws SQLException {
		...
		if (Math.abs(row)>0&&Math.abs(row) <= data.length) {
			this.row = row % (data.length+1);
			return true;
		} else {
	...
	public int findColumn(String columnLabel) throws SQLException {
		...
		for (Map.Entry<String, String> tblStruct : tableStructure.entrySet()) {
			if (tblStruct.getKey().equalsIgnoreCase(columnLabel)) {
				return counter;
			}
			counter++;
		}
	...
	public ResultSetMetaData getMetaData() throws SQLException {//-------------------------------------------->>
		...
		return new MyResultSetMetaData(mydb,data);
	}
	...
	public Object getObject(int columnIndex) throws SQLException {
		...
		if(row > 0&& row <= data.length && columnIndex >0&&columnIndex<=datastruct.size()) {
	...
	public boolean isAfterLast() throws SQLException {
		...
		if (row == data.length+1 && data.length>0) {
			return true;
		}
		return false;
	}
